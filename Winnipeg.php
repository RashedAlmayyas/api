...
$query = "SELECT * FROM WEB_SALES_TAX_INVOICE_VIEW WHERE COMP_NO=2 ORDER BY ID";
$stid = oci_parse($conn, $query);
oci_execute($stid);
while ($invoice_data = oci_fetch_assoc($stid)) {
    $header_id = $invoice_data['ID'];     
    $queryItems = "SELECT * FROM WEB_SALES_TAX_INVOICE_DTL_VIEW WHERE COMP_NO=2 AND HEADER_ID=:header_id";
    ...
    if ($response === false) {
        ...
        if (
            ...
            if (!empty($qrValue1) && !empty($invoice_data['ID'])) {
                ...
                $savePath = '//192.168.0.7/Invoice/tax_Winnipeg/' . $invoiceId . '.jpg';
